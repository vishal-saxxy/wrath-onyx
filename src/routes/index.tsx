...
function EngineMark({ engine }: { engine: (typeof illustrative.engines)[number] }) {
  const logoPaths: Record<(typeof illustrative.engines)[number], string> = {
    ChatGPT: "/logos/chatgpt.svg",
    Gemini: "/logos/gemini.svg",
    Claude: "/logos/claude.svg",
    Perplexity: "/logos/perplexity.svg",
  };
  return <img className="engine-logo" src={logoPaths[engine]} alt="" aria-hidden="true"/>;
}
  ...
  return <section className="home-band hero-band">
    <div className="site-grid hero-grid">
      ...
      <div className="col-span-7 hero-visual" role="img" aria-label={diagramLabels.hero}>
        ...
        <svg className={`hero-paths ${activeEngine ? "has-selection" : ""}`} viewBox="0 0 720 180" aria-hidden="true" focusable="false"><path className={activeEngine && activeEngine !== illustrative.engines[0] ? "is-dimmed" : ""} d="M32 20 C 10 58, 142 84, 194 140"/><path className={activeEngine && activeEngine !== illustrative.engines[1] ? "is-dimmed" : ""} d="M160 20 C 214 52, 220 112, 308 140"/><path className={activeEngine && activeEngine !== illustrative.engines[2] ? "is-dimmed" : ""} d="M288 20 C 272 80, 402 72, 422 140"/><path className={activeEngine && activeEngine !== illustrative.engines[3] ? "is-dimmed" : ""} d="M440 20 C 520 54, 470 104, 536 140"/>{illustrative.engines.map((engine, index) => <circle className={`travelling-dot dot-${index + 1} ${activeEngine && activeEngine !== engine ? "is-dimmed" : ""}`} key={engine} r="4"/>)}</svg>
        <div className="engine-annotation"><span>Multiple AI engines.<br/>One answer.</span><svg viewBox="0 0 260 80" aria-hidden="true" focusable="false"><path d="M250 35C210 49 125 40 25 8m15-5L23 7l12 13"/></svg></div>
        ...
        <div className="hero-annotation"><svg viewBox="0 0 270 112" aria-hidden="true" focusable="false"><path d="M262 103C211 102 130 88 41 54m15-1L39 53l8 14"/></svg><span className="annotation">{illustrative.heroAnnotation}</span></div>
